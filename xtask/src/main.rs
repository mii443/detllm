...
const DETERMINISM_BANNED_PATTERNS: &[(&str, &str)] = &[
    ...
    (
        "Iterator::reduce(", // determinism-allow
        "iterator reductions must be explicitly reviewed or replaced by fixed-order helpers",
    ),
    (
        "Iterator::fold(", // determinism-allow
        "floating-point reductions must use det_num fixed-order helpers or explicit loops",
    ),
    (
        ".fold(", // determinism-allow
        "inferred iterator folds must be explicitly reviewed or replaced by fixed-order helpers",
    ),
    (
        ".try_fold(", // determinism-allow
        "inferred iterator folds must be explicitly reviewed or replaced by fixed-order helpers",
    ),
    (
        "Iterator::try_fold(", // determinism-allow
        "inferred iterator folds must be explicitly reviewed or replaced by fixed-order helpers",
    ),
];
...
fn check_determinism() -> Result<(), String> {
    ...
    if !lock_path.is_file() {
        ...
        let text =
            fs::read_to_string(lock_path).map_err(|e| format!("{}: {e}", lock_path.display()))?;
        scan_lockfile_dependency_policy_text(lock_path, &text, &mut violations);
    }
...
fn scan_lockfile_dependency_policy_text(path: &Path, text: &str, violations: &mut Vec<String>) {
    ...
    for (line_idx, line) in text.lines().enumerate() {
        ...
        if let Some(value) = trimmed.strip_prefix("name = ") {
            let dep_name = value.trim().trim_matches('"');
            if NATIVE_LINK_DEPENDENCY_NAMES.contains(&dep_name) {
                violations.push(format!(
                    "{}:{}: resolved package `{dep_name}` is forbidden because native C/C++ link/build tooling is not allowed",
                    path.display(),
                    line_idx + 1
                ));
            }
...
mod tests {
    ...
    fn determinism_scan_reports_banned_patterns_and_allows_marked_lines() {
        ...
        let fp_ufcs_fold_banned = concat!("Iterator::", "fold(");
        let fp_method_fold_banned = concat!(".fo", "ld(");
        let fp_fold_inferred_banned = concat!(".fo", "ld(seed");
        ...
        let contains_violation = |line: usize, pattern: &str| {
            ...
            violations
                .iter()
                .any(|violation| violation.contains(&location) && violation.contains(pattern))
        };
        assert!(contains_violation(1, banned));
...
"#
    ...
    #[test]
    fn lockfile_policy_rejects_transitive_native_link_tooling() {
        let mut violations = Vec::new();
        let cc_dep = concat!("c", "c");
        let cmake_dep = concat!("cma", "ke");
        let text = format!(
            r#"
...
"#
        ...
        scan_lockfile_dependency_policy_text(Path::new("Cargo.lock"), &text, &mut violations);

        assert_eq!(violations.len(), 2, "{violations:?}");
        for needle in [cc_dep, cmake_dep] {
            assert!(
                violations
                    .iter()
                    .any(|violation| violation.contains(needle)),
