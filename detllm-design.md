...
GGUF標準: スーパーブロック256、16個の16要素サブブロック、`d: f16`、`scales: [i8; 16]`、6bit量子値。内積はllama.cppの `Q6_K x Q8_K` 参照経路に合わせ、入力活性を256要素のQ8_Kに量子化する。Q8_Kは最大絶対値を持つ入力値 `max` の符号を保持し、`iscale = -127.0 / max`、`q[i] = min(127, nearest_int(iscale * x[i]))`、`d = 1.0 / iscale` とする。`nearest_int` はGGMLの整数丸め手順をビット操作で固定する。

`Q6_K` のdotは、各256要素ブロックで8本のi32 laneに整数積を集約し、最後に `(d_w * d_a) * lane_sum` をf32で足し込む。これはQ8A共有対象ではない。
...
**量子化と再量子化のタイミング**: GEMVの直前に、入力ベクトル(f32)を量子化する。`Q8_0` / `Q4_0` / `Q4_K` 重みではQ8Aに量子化する(§3.2)。同じ入力に対する複数のQ8A GEMV(`W_q/W_k/W_v` や `W_gate/W_up`)では、**1回だけ量子化した同一のQ8Aバッファを共有**する(MUST — 量子化回数が変わると結果が変わるため、仕様として固定)。`Q6_K` 重みではQ8_Kに量子化する(§3.5)。
...
- `Q8_0` / `Q4_0` / `Q4_K` Wの場合: `x` をQ8A化 → 各出力行 $r$ について§3.3+§2.2で計算。
- `Q6_K` Wの場合: `x` をQ8_K化 → 各出力行 $r$ について§3.5+§2.2で計算。
